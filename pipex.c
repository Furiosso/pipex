...
static int	manage_processes(t_params *params)
{
	...
	while (++i < params->p_num)
	{
		...
		if (params->pid[i] == 0)
		{
			...
			close_fds(params);
			execute_command(params->index_first_argument + i, params, i);
		}
	}
	close_fds(params);
